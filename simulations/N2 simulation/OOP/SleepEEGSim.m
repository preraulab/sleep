classdef SleepEEGSim < handle
    ...
    methods
        ...
        function obj = addSpindles(obj,ss)
            ...
            if isempty(obj.SpindlesSets)
                obj.SpindlesSets = ss;
            else
                obj.SpindlesSets(end+1) = ss;
            end
        end
        ...
        function sim(obj,T)
            ...
            obj.Slow_Waves.sim(t);
            signal = signal + obj.Slow_Waves.Signal;
            [obj.SO_phase, obj.SO_EEG] = obj.computeSOPhase(obj.Aperiodic.Signal+obj.Slow_Waves.Signal);

            for ii = 1:length(obj.SpindlesSets)
                obj.SpindlesSets(ii).sim(t,obj.SO_phase);
                signal = signal + obj.SpindlesSets(ii).Signal;
            end

            for ii = 1:length(obj.LineNoiseSets)
                obj.LineNoiseSets(ii).sim(t);
            ...
            figure
            multitaper_spectrogram(signal,obj.Fs,[0 25], [15 29], [30,5]);
            scrollzoompan;
        end

        function plot(obj)
            T = length(obj.Signal)/obj.Fs;
            t = 0:1/obj.Fs:T;
    ...
    warning('off')
    [spect, stimes, sfreqs] = multitaper_spectrogram_mex(obj.Signal, obj.Fs, [.5 obj.Fs/2], [2 3], [1 .05], 2^10,'constant','plot_on',false);
    warning('on')
    if ~isempty(spindle_opts)
        [t_sp, b, yhat, dylo, dyhi] = fit_ppsplines(t, obj.SO_phase, sim_features.spindles, spindle_opts);
    end
