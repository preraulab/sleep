classdef Oscillator < handle
    ...
    %       a: double - Damping parameter (Default: 0.9)
    ...
    %       a: double - Autoregressive parameter
    ...
    properties
        Frequency double {mustBeReal, mustBeVector, mustBeNonempty} = 1; % Frequency range for band power [Hz]
        sig2state double {mustBePositive, mustBeReal, mustBeNonempty} = 1; % Amplitude of state noise
        sig2obs double {mustBePositive, mustBeReal, mustBeNonempty} = 0.1; % Amplitude of observation noise
        a double {mustBePositive, mustBeReal, mustBeNonempty} = 0.9; % Autoregressive parameter
    ...
    methods
        function obj = Oscillator(varargin)
            ...
            addParameter(p, 'a', obj.a, @(x) validateattributes(x, {'numeric'}, {'scalar', 'positive'}));
        ...
        function oscillation = sim(obj, t)
            Fs = 1/(t(2)-t(1));
            oscillation = obj.genNoisyOscillator(t, Fs, obj.Frequency, obj.a, obj.sig2state, obj.sig2obs);
            obj.Signal = oscillation;
        end
    end

    methods (Static)
        function y = genNoisyOscillator(t, Fs, freq, a, s2state, s2obs)
