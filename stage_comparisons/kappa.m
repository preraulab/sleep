...
function [k, h, p, STATS] = kappa(x,varargin)
...
%
% Syntax: 	[STATS, p, h] = kappa(X,W,ALPHA, verbose)
%
%     Inputs:
...
%
% Edited by Michael J. Prerau, PhD 7/27/2020

%Input Error handling
p = inputParser;
addRequired(p,'x',@(x) validateattributes(x,{'numeric'},{'square','nonempty','integer','real','finite','nonnan','nonnegative'}));
addOptional(p,'w',0, @(x) isnumeric(x) && isreal(x) && isfinite(x) && isscalar(x) && ismember(x,[-1 0 1 2 3]));
...
parse(p,x,varargin{:});
x=p.Results.x; w=p.Results.w; alpha=p.Results.alpha; verbose = p.Results.verbose;
